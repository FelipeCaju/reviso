...
import { Plus, ChevronLeft, ChevronRight, CalendarDays, Car } from "lucide-react";
...
import { AppointmentStatusBadge, appointmentStatusInfo, appointmentTypeInfo } from "@/components/StatusBadge";
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <Button size="sm" variant={weekStart === thisWeekStart ? "default" : "outline"} onClick={() => setRefDay(todayISO())}>
          Esta semana
        </Button>
        <Button size="sm" variant={weekStart === nextWeekStart ? "default" : "outline"} onClick={() => setRefDay(nextWeekStart)}>
          Próxima semana
        </Button>
        <div className="flex items-center gap-1 ml-auto">
          <Button size="icon" variant="outline" onClick={() => setRefDay(addDaysISO(-7, new Date(weekStart + "T00:00:00")))}>
            ...
            return (
              <div key={d} className="w-40 md:w-auto shrink-0">
                ...
                <div className="border border-t-0 border-border rounded-b-lg bg-card min-h-[120px] p-1.5 space-y-1.5">
                  {list.length === 0 ? (
                    <button onClick={() => { setFormPrefill({ scheduled_date: d }); setFormOpen(true); }} className="w-full text-xs text-muted-foreground py-4 hover:bg-accent rounded">
                      + agendar
                    </button>
                  ) : list.map((a) => (
                    <button
                  ...
                  <button onClick={() => { setFormPrefill({ scheduled_date: d }); setFormOpen(true); }} className="w-full text-xs text-muted-foreground py-1.5 hover:bg-accent rounded border border-dashed border-border">
                    + agendar
                  </button>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {detail.status !== "confirmado" && (
                    <Button variant="outline" onClick={() => updateStatus(detail, "confirmado")}>Confirmar</Button>
                  )}
                  {detail.status !== "veiculo_recebido" && detail.status !== "em_atendimento" && detail.status !== "concluido" && (
                    <Button onClick={() => updateStatus(detail, "veiculo_recebido", { actual_arrival: new Date().toISOString() })}>
                      Veículo Chegou
                    </Button>
                  )}
                  {detail.status === "veiculo_recebido" && (
                    <Button onClick={() => updateStatus(detail, "em_atendimento")}>Iniciar Atendimento</Button>
                  )}
                  {detail.status !== "concluido" && detail.status !== "cancelado" && (
                    <Button variant="outline" onClick={() => updateStatus(detail, "concluido")}>Concluir</Button>
                  )}
                  {detail.status !== "nao_compareceu" && detail.status !== "concluido" && (
                    <Button variant="outline" onClick={() => updateStatus(detail, "nao_compareceu")}>Não Compareceu</Button>
                  )}
